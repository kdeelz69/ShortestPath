...
model = joblib.load("generalized_delay_predictor.pkl")
print("✅ Generalized delay predictor model loaded!")
...
def predict_traffic_lights(G, model):
    ...
    for node in G.nodes():
        try:
            ...
            delay = 5  # fallback default delay
        delays[node] = delay
    return delays
...
# Evaluate and log route comparison
def evaluate_path_accuracy(map_obj, planner_with_delays, planner_without_delays, traffic_lights, log_file):
    ...
    with open(log_file, 'w', newline='') as csvfile:
        ...
        writer.writerow(["RouteType", "Distance(km)", "TotalTime(min)", "Delay(seconds)"])
        writer.writerow(["Delay-Aware", round(dist_delay, 2), round(time_delay / 60, 2), round(delay_time, 1)])
        writer.writerow(["Distance-Only", round(dist_base, 2), round(time_base / 60, 2), 0])

    print("📊 Comparison logged to", log_file)
...
if __name__ == "__main__":
    ...
    if planner_with.path and planner_without.path:
        ...
        evaluate_path_accuracy(map_obj, planner_with, planner_without, traffic_lights, "route_comparison_results.csv")
    else:
